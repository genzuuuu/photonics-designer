...
def write_sparameters_lumerical(
    ...
    **settings,
    ...
    if (count == 0): component_extended_beyond_pml.name = "top"
    else: component_extended_beyond_pml.name = f"top${count}"
    ...
    for layer, thickness in layer_to_thickness.items():
        ...
        if (count==0): s.gdsimport(str(gdspath), "top", f"{layer[0]}:{layer[1]}")
        else: s.gdsimport(str(gdspath), f"top${count}", f"{layer[0]}:{layer[1]}")
